...
def get_model_info():
    try:
        res = requests.get(f"{API_URL}/model-info", timeout=30)
...
# ── Fetch data ONCE here — used in both columns ──────────────────────────────────
...
with left_col:
    if forecast_data is not None and historical_data is not None:
        ...
        fig.update_layout(
            ...
            legend=dict(bgcolor="#1e2130", bordercolor="#3d4466"),
            hovermode="x unified",
            height=400
        )
...
with right_col:
    ...
    if forecast_data is not None:
        ...
        forecast_table_df["Predicted Revenue (£)"] = forecast_table_df["Predicted Revenue (£)"].apply(
            lambda x: f"£{x:,.2f}"
        )
